...
Elm.Main.make = function (_elm) {
   ...
   var signalMessageOnEnter = F2(function (enterPressed,
   message) {
      return _U.eq(enterPressed,
      true) ? message : "";
   });
   var renderSpiningMessage = F3(function (message,
   angle,
   spinningHeight) {
   ...
   var renderOuterVoid = F3(function (voidAngle,
   width,
   height) {
      return A2($Graphics$Collage.rotate,
      $Basics.degrees(voidAngle),
   ...
   _L.fromArray([A2($Html.h1,
                ...
                _L.fromArray([$Html$Attributes.style(_L.fromArray([{ctor: "_Tuple2"
                                                                  ...
                                                                  ,{ctor: "_Tuple2"
                                                                   ,_0: "font-family"
                                                                   ,_1: "Creepster"}]))]),
                _L.fromArray([$Html.text("into the void")]))]));
   var messageMailbox = $Signal.mailbox("");
   var inputArea = F2(function (message,
   show) {
      return function () {
         var visibleInputStyles = _L.fromArray([{ctor: "_Tuple2"
                                                ,_0: "display"
                                                ,_1: "block"}
                                               ,{ctor: "_Tuple2"
                                                ,_0: "width"
                                               ...
                                               ,{ctor: "_Tuple2"
                                                ,_0: "font-size"
                                                ,_1: "32pt"}]);
         var inputStyles = show ? _L.fromArray([{ctor: "_Tuple2"
                                                ,_0: "display"
                                                ,_1: "none"}]) : visibleInputStyles;
         return A2($Html.div,
         _L.fromArray([$Html$Attributes.style(_L.fromArray([{ctor: "_Tuple2"
                                                            ,_0: "position"
                                                            ,_1: "absolute"}
         ...
         _L.fromArray([A2($Html.input,
                      _L.fromArray([$Html$Attributes.style(inputStyles)
                                   ,$Html$Attributes.placeholder("Type your feelings here")
                                   ,$Html$Attributes.value(message)
                                   ,A3($Html$Events.on,
                                   "input",
                                   $Html$Events.targetValue,
                                   $Signal.message(messageMailbox.address))]),
                      _L.fromArray([]))
                      ,A2($Html.div,
                      _L.fromArray([$Html$Attributes.style(_L.fromArray([{ctor: "_Tuple2"
                                                                         ,_0: "position"
                      ...
                      _L.fromArray([$Html$Attributes.href("http://screamintothevoid.com/")]),
                      _L.fromArray([$Html.text("An Elm powered homage to http://screamintothevoid.com/")]))]))]));
      }();
   });
   var renderTheVoid = F4(function (message,
   time,
   _v0,
   _v1) {
      return function () {
         ...
         {case "_Tuple2":
            return function () {
                 ...
                 {case "_Tuple2":
                    return function () {
                         var timeSinceEnterPressed = $Basics.round((time - _v0._0) / 100);
                         var shrinkingHeight = $Basics.toFloat(72 - 3 * timeSinceEnterPressed);
                         var spiningMessageAngle = $Basics.toFloat(60 + 24 * timeSinceEnterPressed);
                         var showInput = _U.cmp(timeSinceEnterPressed,
                         40) < 0;
                         var spiningMessage = showInput ? message : "";
                         var voidAngle = $Basics.toFloat(A2($Basics._op["%"],
                         $Basics.round(6 * $Time.inSeconds(time)),
                         360));
                         return A2($Html.div,
                         _L.fromArray([]),
                         _L.fromArray([css("https://fonts.googleapis.com/css?family=Creepster")
                                      ,heading
                                      ,A2(inputArea,message,showInput)
   ...
   $Time.every(50 * $Time.millisecond),
   $Time.timestamp($Keyboard.enter),
   $Window.dimensions);
   var shoutOut = Elm.Native.Port.make(_elm).outboundSignal("shoutOut",
   function (v) {
      return v;
   },
   A3($Signal.map2,
   signalMessageOnEnter,
   $Keyboard.enter,
   messageMailbox.signal));
   _elm.Main.values = {_op: _op
                      ...
                      ,inputArea: inputArea
                      ,css: css
                      ,renderTheVoid: renderTheVoid
                      ,renderOuterVoid: renderOuterVoid
                      ,renderSpiningMessage: renderSpiningMessage
                      ,signalMessageOnEnter: signalMessageOnEnter};
   return _elm.Main.values;
};
Elm.Maybe = Elm.Maybe || {};
